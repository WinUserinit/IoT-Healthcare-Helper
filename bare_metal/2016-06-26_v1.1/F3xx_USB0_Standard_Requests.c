...
{                                      // features on endpoint 1
   ...
   // # pragma message("writeReadHID() EP3IN  256-byte buffer")
            ...
            // VERIFY: OS24EVK-66 USB_EP_INDEX 1 for EP_STATUS[USB_EP_INDEX] and POLL_WRITE_BYTE (INDEX, USB_EP_INDEX)
            POLL_WRITE_BYTE (INDEX, USB_EP_INDEX);// Clear feature endpoint 1 halt
            POLL_WRITE_BYTE (EINCSR1, rbInCLRDT);
            EP_STATUS[USB_EP_INDEX] = EP_IDLE;    // Set endpoint 1 status back to idle
         }
      }
      else
      {
...
{                                      // feature for endpoint 1
   ...
   // # pragma message("writeReadHID() EP3IN  256-byte buffer")
         ...
         {
            POLL_WRITE_BYTE (INDEX, 3);// Set feature endpoint 1 halt
            POLL_WRITE_BYTE (EINCSR1, rbInSDSTL);
            EP_STATUS[3] = EP_HALT;
         }
#endif // USB_writeReadHID_EP1OUT_EP1IN
...
{                                      // and size to correct descriptor and
   ...
   {                                   // was requested, and set data ptr and
	  ...
	  case DSC_STRING:
         ...
         // Assuming global STRINGDESCTABLE[] has 3 entries, 
         // so STRINGDESCTABLE[2] is the last valid string descriptor
   ...
   // # pragma message("writeReadHID() EP3OUT 256-byte buffer")
   // # pragma message("writeReadHID() EP3IN  256-byte buffer")
         if ( (SETUP.wValue.c[LSB] == IN_EP3) )
         {
            DATAPTR = (uint8_t*) &Endpoint1Desc;
            DATASIZE = Endpoint1Desc.bLength;
         }
         else if ( (SETUP.wValue.c[LSB] == OUT_EP3) )
         {
            DATAPTR = (uint8_t*) &Endpoint2Desc; // the OUT endpoint, not necessarily EP2
            DATASIZE = Endpoint2Desc.bLength;
         }
...
{                                      // current device configuration value
   ...
   else
   {
      ...
      {                                // results in configuration being set
         ...
         // VERIFY: OS24EVK-66 USB_EP_INDEX 1 for EP_STATUS[USB_EP_INDEX] and POLL_WRITE_BYTE (INDEX, USB_EP_INDEX)
         EP_STATUS[USB_EP_INDEX] = EP_IDLE;       // Set endpoint status to idle (enabled)

         POLL_WRITE_BYTE (INDEX, USB_EP_INDEX);   // Change index to endpoint 1
...
void Get_Report (void)
{
   ...
   DATAPTR = IO_BUFFER.Ptr;
   DATASIZE = IO_BUFFER.Length;
